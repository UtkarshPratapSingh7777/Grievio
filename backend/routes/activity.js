...
ActivityRouter.get("/staff", staffTokenVerify, async (req, res) => {
  try {
    const items = await Activitymodel.find({ userType: "staff", userId: req.staff._id })
      .sort({ createdAt: -1 })
      .limit(20);
    res.status(200).send({ items });
  } catch (err) {
...
ActivityRouter.get("/admin", adminTokenVerify, async (req, res) => {
  try {
    const items = await Activitymodel.find({ userType: "admin", userId: req.admin._id })
      .sort({ createdAt: -1 })
      .limit(20);
    res.status(200).send({ items });
  } catch (err) {
